"""EnCodec audio codec for MLX.

Neural audio codec using SEANet encoder/decoder with Residual Vector Quantization.
Used by MusicGen for tokenizing and detokenizing audio at 32kHz.

Architecture:
    - SEANet Encoder: strided Conv1d downsampling (ratios [8,5,4,4], total stride 640)
    - LSTM bottleneck: 2-layer LSTM with custom Metal kernel
    - Residual VQ: 4-16 codebooks of size 2048, codebook_dim=128
    - SEANet Decoder: ConvTranspose1d upsampling (mirror of encoder)

Frame rate: 32000 / 640 = 50 Hz

Ported from Apple's mlx-examples/encodec (Apache 2.0 License).
...
def _materialize(*args):
    """Force MLX lazy graph materialization on GPU (wraps mlx.core function)."""
    mx.eval(*args)  # noqa: S307


# ---------------------------------------------------------------------------
# Custom Metal LSTM kernel for GPU-accelerated sequential processing
# ---------------------------------------------------------------------------
...
class LSTM(nn.Module):
    """LSTM using custom Metal kernel for GPU acceleration."""
    ...
    def __call__(self, x, hidden=None, cell=None):
        ...
        for t in range(x.shape[-2]):
            ...
            hidden, cell = _lstm_custom(x, hidden, cell, t)
...
# ---------------------------------------------------------------------------
# Convolution wrappers with padding logic
# ---------------------------------------------------------------------------


class EncodecConv1d(nn.Module):
    """Conv1d with asymmetric or causal padding and optional normalization."""
    ...
    def _get_extra_padding_for_conv1d(self, hidden_states: mx.array) -> int:
...
class EncodecConvTranspose1d(nn.Module):
    """ConvTranspose1d with asymmetric or causal padding and optional normalization."""
...
# ---------------------------------------------------------------------------
# SEANet building blocks
# ---------------------------------------------------------------------------


class EncodecLSTM(nn.Module):
    """Residual LSTM bottleneck."""
...
class EncodecEncoder(nn.Module):
    """SEANet encoder: waveform -> continuous embeddings."""
...
class EncodecDecoder(nn.Module):
    """SEANet decoder: continuous embeddings -> waveform."""
            ...
            for j in range(config.num_residual_layers):
                model += [
                    EncodecResnetBlock(
                        config,
                        current_scale // 2,
                        (config.dilation_growth_rate**j, 1),
                    )
                ]
            scaling //= 2
...
class EncodecResidualVectorQuantizer(nn.Module):
    """Residual Vector Quantizer (RVQ) — multiple codebooks for residual coding."""

    def __init__(self, config):
        super().__init__()
        self.codebook_size = config.codebook_size

        hop_length = int(np.prod(config.upsampling_ratios))
    ...
    def encode(
        ...
        """Encode embeddings into RVQ codes."""
        ...
        return mx.stack(all_indices, axis=1)
...
class EncodecModel(nn.Module):
    """EnCodec: Neural audio codec with encoder, decoder, and RVQ.

    Supports encoding audio to discrete tokens and decoding tokens back to audio.
    MusicGen uses this for the decode path (tokens -> waveform).
    """
    ...
    def encode(
        ...
        Args:
            input_values: Audio (batch, time, channels).
            padding_mask: Mask for padded regions.
            bandwidth: Target bandwidth in kbps.

        Returns:
            Tuple of (codes, scales).
        """
        if bandwidth is None:
            bandwidth = self.config.target_bandwidths[0]
        if bandwidth not in self.config.target_bandwidths:
            raise ValueError(
                f"Bandwidth {bandwidth} not supported. "
                f"Choose from {self.config.target_bandwidths}."
            )
        ...
        if (input_length % stride) != step:
            raise ValueError(
                "Input length is not properly padded for batched chunked encoding."
            )
    ...
    def decode(
        ...
        Args:
            audio_codes: Codes of shape (batch, num_codebooks, frames).
            audio_scales: Scaling factors for each chunk.
            padding_mask: Optional mask to truncate output.
        ...
        if chunk_length is None:
            if audio_codes.shape[1] != 1:
                raise ValueError(f"Expected one frame, got {audio_codes.shape[1]}")
    ...
    def from_pretrained(cls, path_or_repo: str) -> Tuple["EncodecModel", Callable]:
        ...
        Returns:
            Tuple of (model, preprocessor_fn).
        """
...
def preprocess_audio(
    ...
    """Prepare audio inputs for the EnCodec model.

    Args:
        raw_audio: Audio waveform(s).
        sampling_rate: Expected sample rate.
        chunk_length: Chunk length for encoding.
        chunk_stride: Chunk stride for encoding.

    Returns:
        Tuple of (padded_audio, padding_mask).
    """
